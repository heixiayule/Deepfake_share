...
BATCH = 1
INPUT_SHAPE = (128, 128, 1)   # 可修改

model = build_lcnn_lstm(list(INPUT_SHAPE), n_label=2)
spec = (tf.TensorSpec((BATCH, *INPUT_SHAPE), tf.float32, name="input"),)
_ = model(tf.random.normal([BATCH, *INPUT_SHAPE]), training=False)

out_path = os.path.join(EXPORT_DIR, "lcnn_lstm.onnx")
win_path = os.path.join(WIN_DIR, "lcnn_lstm.onnx")
